...
var Server = module.exports = {
  ...
  init: function(config, options){
    ...
    Server.server.get('*', function(req, res){
      ...
      var pathToServe = pathArray.pop();

      while(true){
        var pathToTry = path.join( config.basePath, pathToServe );
        if( fs.existsSync(pathToTry) ){
          break;
        }

        pathToServe = pathArray.pop() + path.sep + pathToServe;
      ...
      fullUrl = fullUrl.split('/').pop();
      var fullPath = path.join(config.basePath, pathToServe);
...
var setupStaticFiles = function(config){
  Server.server.get(['bundle*', '*/bundle*'], function(req, res, next){
    ...
    var ext = path.extname(req.url);//HACK FOR RICK
    served =  path.join(config.basePath, 'bundles/_dev' + ext);//HACK FOR RICK
    res.set( {'Content-Type': mime.lookup(served) } )
    fs.createReadStream( served )
      .pipe( res );
  })

  Server.server.get('*/naked-gaston.js', function(req, res){
