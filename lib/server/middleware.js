...
module.exports = function middleware(config, Server){
  return function(req, res, next){
    if( path.extname(req.url) || !running[req.url]){
      return next(); 
    }

    var dirPath = path.join( config.basePath, req.url );

    switch( running[req.url] ){
      case 'dev': 
        return Bundler.forDev(dirPath)
          .then(function(watchify){
            watchifies[dirPath] = watchify;
          })
...
module.exports.register = function(runPath, action){
