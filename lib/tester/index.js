...
var testNode = function(reqPath, testPath){
  ...
  return getFiles( reqPath || testPath )
    .then(function(files){
      console.log('adding files now:', files)
      for(var i = 0, l = files.length; i < l; i++){
        var file = files[i];
        console.log( 'adding file', file, 'to mocha' );
...
var testBrowser = function(reqPath, testPath){
  ...
  var globFilter = getGlobFilter(reqPath, testPath);
  console.log('globFilter', globFilter);
...
var compile = function(files){
  ...
  return new Promise(function(fulfill, reject){
    console.log('adding files now to browserify:')
...
var getGlobFilter = function(reqPath, testPath){
  ...
  if(!reqPath){
    ...
    console.log( testRoot, 'exists?', fs.existsSync(testRoot) )
...
function getFiles(basePath){
  ...
  (function recur(base){
    var list = fs.readdirSync(base); console.log(list)
