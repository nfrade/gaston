...
var Tester = module.exports = function(options){
  ...
  for(var i = 0, l = self.runners.length; i < l; i++){
    ...
    if(!Tester.runners[runnerName]){
      try {
        var runner = require(runnerName);
        Tester.runners[runnerName] = require( runnerName );
      } catch(ex) {
        throw Error('no such test runner: ' + runnerName);
      }        
    }
  }
};
...
Tester.prototype.run = function(){
  ...
  // [TODO] how to do this?
  // if(self.stream){ console.log('here', self.stream)
  //   process.stdout.on('data', function(buf){ console.log('data');
  ...
  return self.runners.reduce(function(a, b){
    ...
    return a.then( function(errors){
      return runner(self.source, errors);
    });
  }, Promise.resolve(0) )
  .catch(function(err){
    console.log(err);
