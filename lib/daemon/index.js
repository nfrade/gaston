...
var compileDashboard = function compileDashboard() {
  return new Promise(function (resolve, reject) {
    var dashboardPath = path.join(__dirname, '..', 'bundler/dashboard')
    var bundleOptions = {
    ...
    return bundler.bundle(true)
      .then(function (bundle) {
        ...
        var gastonPath = path.join(tmpdir, 'gaston-dashboard')
        var indexPath = path.join(__dirname, '../../', 'gaston-files', 'index.html')

        if (!fs.existsSync(gastonPath)) {
          fs.mkdirSync(gastonPath)
        }

        return fs.writeFileAsync(path.join(gastonPath, 'bundle.js'), bundle.js, 'utf8')
          .then(() => fs.writeFileAsync(path.join(gastonPath, 'bundle.css'), bundle.css, 'utf8'))
          .then(() => fs.readFileAsync(indexPath, 'utf8')
            .then((data) => {
              ...
              return fs.writeFileAsync(path.join(gastonPath, 'index.html'), data, 'utf8')
            }))
          .then(() => ncp(path.join(dashboardPath, 'assets'), path.join(gastonPath, 'assets')))
      })
      .then(resolve)
      .catch(reject)
  })
