...
var Compiler = module.exports = {
  ...
  compile: function(){
    ...
    var gastonBrowserTransform = require('./gaston-browser-transform')(Bundler);
    ...
    if(Bundler.building){
      ...
      Compiler.browserify.require( browserGastonPath, { expose: 'gaston' } );
    }
...
var injectPackageJSON = function(wStream){
  return repo.getBranch(Bundler.dirPath)
    .then(function(branch){
      ...
      Bundler.Watcher.addWatcher(pkgPath);
      pkgStream
        .pipe(inform)
        .pipe(wStream);
    });
...
var setSelfAlias = function(){
  var aliases = {};
  if(Bundler.pkg.gaston && Bundler.pkg.gaston['require-paths']){
    aliases = _.extend(aliases, Bundler.pkg.gaston['require-paths'], function(v, o){
      return path.join(Bundler.basePath, o);
    });
  }
  aliases[Bundler.pkg.name] = Bundler.basePath;
  aliases['~'] = Bundler.basePath;
