...
Transform.render = function(){
  var requires = [];
  for(var i in Transform.requires ){
    var reqArray = Transform.requires[i];
    if(reqArray){ 
      requires = requires.concat(reqArray);
    }
  }

  var input = Transform.input.reduce(function(a, b){
    if( ~requires.indexOf(b.src) ){
      return a + '\n/* ' + b.src + ' */\n' + b.input + '\n\n';
    } else {
      return a;
    }
...
var handleJsFile = function(file, str, options){
  var requires = Transform.requires;
  var match;
  requires[file] = null;
  while( match = requireRegExp.exec(str) ){
    var required = match[3];
    var extname = path.extname(required);
    if( ~CSS_EXTENSIONS.indexOf(extname) ){
      if(required.indexOf('./') === 0){
        required = required.replace('.', path.dirname(file));
      }
      requires[file] = requires[file] || [];
      requires[file].push( required );
    }
  };
};
