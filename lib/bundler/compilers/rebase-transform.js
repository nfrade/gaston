...
var Transform = module.exports = function(file, options){ 
  return through(function(buf, enc, next){
    ...
    Transform.input.push( {
      src: file.replace(options.basePath),
      input: str
    } );
...
Transform.render = function(){
  ...
  // console.log(Transform.requires);
  return less.render(input, {
    paths: Transform.paths, compress: false, strictMath:true
  });
};

var logged = false;
var gatherCssRequires = function(file, str){
  var match, requires = Transform.requires;
  while( match = requireRegExp.exec(str) ){
    var required = match[3];
    if( ~CSS_EXTENSIONS.indexOf( path.extname(required) ) ){
