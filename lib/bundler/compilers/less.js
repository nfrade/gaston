...
var rebaseURLs = function(allCss){
  //var regExp = /url(\ +)?\((\ +)?['"](directv-fl)\/(.+)['"](\ +)?\)/g;

  var rex = new RegExp('url(\\ +)?\\((\\ +)?[\'"]('+Bundler.pkg.name+')\/(.+)[\'"](\\ +)?\\)', 'g')

  // var match = rex.exec(allCss);
  // delete match.input;
  // console.log('match', match);

  return allCss.replace(rex, 'url("$4x")')

  var projectName = Bundler.pkg.name;
  var basePath = Bundler.basePath;
