...
var Bundler = module.exports = {
  forDev: function(dirPath){
    var bundleFileName = '_' + dirPath.replace(config.basePath, '').replace(/\//g, '_') + 'bundle';
    var jsPath = path.join( bundlesPath, bundleFileName + '.js' );
    var cssPath = path.join( bundlesPath, bundleFileName + '.css');

    return new Promise(function(fulfill, reject){
      log.info( 'bundler', 'compiling JS for', dirPath.replace(config.basePath, '') );

      var jsStream = fs.createWriteStream(jsPath);
      jsStream.on('close', function(){
      ...
      b.transform( blessify, getBlessifyOptions() );
...
var getBlessifyOptions = function(){
