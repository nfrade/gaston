...
var Bundler = module.exports = {
  bundle: function(dirPath, action, fileToCompile){
    ...
    return new Promise(function(fulfill, reject){
      ...
      var mapPath = path.join( bundlesPath, bundleFileName + '.js.map');
      bundle
        .pipe( exorcist(mapPath) )
        .pipe( jsStream );
    })
    .then(function(){
      dirPath = 'some-path';//HACK FOR RICK
...
var applyTransformsAndRequires = function applyTransforms(b, dirPath, action, fileToCompile){
  ...
  if(action === 'dev' || action === 'test'){
    ...
    toCompile = fileToCompile;//HACK FOR RICK
    b.require(toCompile , {
      expose: 'index.js'
    });
  ...
  config.gaston.browserify = config.gaston.browserify || {};
