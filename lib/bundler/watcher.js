...
var watcherHandler = function(fullPath){
  return function(curr, prev){
    ...
    var prettyPath = fullPath.replace(config.basePath, '');
    log.info('watcher', 'changed', prettyPath);
    SocketServer.broadcast('server-message', {type: 'watcher', message: 'file changed', file: prettyPath });
...
var compileJS = function(fullPath){
  ...
  return Bundler.compilerPromise
    .then(function(){
      SocketServer.broadcast('server-message', 'JS compiled successfully ')
    });
}
...
var compileCSS = function(){
  Bundler.compilerPromise = Bundler.cssCompiler.compile(Bundler.cssFileMappings)
    .then(function(cssToWrite){
      return Bundler.writeCssToBundle(cssToWrite)
    })
    .then(function(){
      SocketServer.broadcast('server-message', 'CSS compiled successfully ')
    });
  return Bundler.compilerPromise;
}
...
var browserReload = function(){
  if(!config.noAutoReload){
    SocketServer.broadcast('server-message', 'Reloading Clients')
    setTimeout(function(){
