...
var createDirectories = function(){
  ...
  return mkdirp( bundlePath )
    .then(function(){
      if( isTest ){
        return mkdirp( path.join(bundlePath, 'vendor') )
      }
    })
      ...
      return fs.readFileAsync( path.join(filesPath, indexFile), 'utf8' )
        ...
        .then( function(exists){
          if(!exists){ 
            ...
            content = content.replace( /\{\{mochapath\}\}/g, path.join(config.bundle, 'vendor', 'mocha') );
            return fs.writeFileAsync( path.join(process.cwd(), 'index.html'), content, 'utf8')
          }
        })
        .then(function(){
          if(isTest){
            var source = path.join(__dirname, '../..', 'node_modules', 'mocha');
            var dest = path.join(process.cwd(), config.bundle, 'vendor', 'mocha')
