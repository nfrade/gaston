...
var HttpServer = module.exports = function (project) {
  var self = this
  self.listening = false
  self.ip = project.ip
  self.port = project['http-port']
  self.connections = []
  self.basePath = project['base-path']
  self.app = express()
  ...
  var serveIndex
  if (!project.project) {
    serveIndex = ServeIndex(self.basePath, serveIndexOptions)
    self.app.use(middleware.addProject)
    self.app.use(serveIndex)
  } else {
    serveIndexOptions.isProject = true
    serveIndex = ServeIndex(self.basePath, serveIndexOptions)
    self.app.use(serveIndex)
  }
