...
var Server = module.exports = {
  ...
  init: function(options){
    ...
    Server.app = express();
    Server.port = options.port || config['http-port'];
    Server.basePath = options.basePath || getBasePath(null, config);
  ...
  start: function(options){
    return new Promise(function(resolve, reject){
      if( Server.listening ){
        return reject( new Error( 'http server already running on port ' +  httpServer.address().port ) );
      }
      Server.init(options);
  ...
  launch: function(config, path){
    var address = 'http://' + Server.ip + ':' + config.port + '/';
    if(Server.isTesting){
      address += 'test/';
    }
...
var setupStaticFiles = function(options){
  ...
  Server.app.get('*/naked-gaston.js', function(req, res){
