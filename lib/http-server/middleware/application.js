...
var getIndexHTML = function (app) {
  ...
  if (app.options['index-path']) {
    indexPath = app.options['index-path']
    if (!path.isAbsolute(indexPath)) {
      indexPath = path.join(app.project, indexPath)
    }
  }
  ...
  app.indexStream = fs.createReadStream(indexPath)
    .pipe(replacer({
      ...
      '$bundle.css': 'bundle.css?app=' + app.basePath,
      '$bundle.js': 'bundle.js?app=' + app.basePath
    }))
  return app
}
